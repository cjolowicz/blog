...
Installing [Hugo](https://gohugo.io/) on macOS is easily achieved
...
In this section we set up the `blog` repository on GitHub.
...
Initialize a git repository in the newly created directory, and create
the initial commit:
...
recommended in Hugo's Quick Start tutorial. To install the theme, we
make use of
...
already been generated for us. Edit this file to set the site URL and
the blog title, and to declare the theme we just installed. This is
...
We are now ready to publish the `blog` repository to GitHub. One
...
In this section, we set up another repository, named
`username.github.io`, for the static content generated by
Hugo. [GitHub Pages](https://pages.github.com/) will deploy this
repository automatically to the site located at
...
empty repository cannot be added as a git submodule, which is what we
are about to do in a second.
...
If you browse to https://username.github.io/ at this point, you will
see that the site is already live, using the contents of
`README.md`. This is going to be replaced by Hugo-generated content as
we finish this walkthrough.

#### Cleaning up

You can now safely remove your local clone of `username.github.io`, as
...
We are almost done with the repository setup. Our final step is to
link the `username.github.io` repository to the `blog` repository, by
making the former a _git submodule_ of the latter.
...
Travis CI will need write access to the `username.github.io`
repository to be able to push the generated content to it. Instead of
granting the CI job access to our personal GitHub account, and thus to
all of our repositories, we will set up a separate bot account with
...
The third step will be delegated to a shell script, which we add in
the next section.
...
remove the generated files before running the deployment script.

#### Adding the deployment script

Create the script `deploy.sh` in the `blog` repository, again
...
You can also invoke this script manually on your machine, after
running `hugo` to rebuild the site. Outside of CI, the script will use
your normal GitHub credentials to push and deploy the generated
content.

#### Finishing

Finally, commit the added files and push them to the `blog`
repository.
...
commit in the submodule. It cannot, because the bot account does not
have write access to the `blog` repository.

This is also not really an issue, because our site is deployed
directly from `username.github.io`, rather than from the `blog`
repository's submodule.

## Writing a post

Blog posts can be written using
[Markdown](https://gohugo.io/content-management/formats/) syntax, with
a [YAML](https://gohugo.io/content-management/front-matter/) preamble
called _front matter_.

Use the Hugo CLI to generate an initial file:
