...
- `C` are violations reported by [mccabe](https://github.com/PyCQA/mccabe),
  which checks the
  [complexity](https://en.wikipedia.org/wiki/Cyclomatic_complexity) of your
  Python code against the limit specified via the `max-complexity` option.
...
flake8](https://github.com/DmytroLitvinov/awesome-flake8-extensions). Some of
these are presented in a separate section at the end of this guide.
...
Exclude Black from the sessions run by default, by setting
...
Instead, check adherence to the Black code style as part of the linter session,
...
Some built-in `flake8` warnings do not align well with Black. Configure `flake8`
to ignore `E203` (Whitespace before ':'), and `W503` (Line break before binary
...
But don't do this manually. Let's automate the release process.
