...
- an extra module with a function
- API documentation (see docker-py)
...
Welcome to the *tour de force* of the Python ecosystem in late 2019! Python 3.8
has been [officially released](https://docs.python.org/3/whatsnew/3.8.html) last
week, and the [Python 2 sunset](https://www.python.org/doc/sunset-python-2/)
will occur on new year 2020, after more than a decade of coexistence with Python
3.
...
license](https://choosealicense.com/licenses/mit/). Clone the repository to your
machine, and `cd` into it:
...
Use `git status` to discover files generated by commands shown in the tutorial.

> *Throughout this tutorial, replace `hypermodern-python` with the name of your
> own repository. Choose a different name to avoid name collisions on PyPI and
> Read the Docs.*
...
virtualenv, and invoke the script directly:
...
[pylint](https://www.pylint.org) and [flake8](http://flake8.pycqa.org). We opt
for the latter due to its ease of setup and use.
...
Add a GitHub Actions badge to your repository page, by adding the following
to the top of your `README.md`:
...
Add the documentation link to your package metainfo:
...
Don't do this manually. You should automate the release process:

- Build the package on every push to GitHub, for testing.
- Upload the package when a Git tag is pushed.
...
You can now trigger package uploads by pushing a release tag to GitHub. 
...
4. **Create a GitHub release**
    ...
    After pushing the release tag, create a [GitHub
    release](https://help.github.com/en/articles/creating-releases). If your
    code changes come in via Pull Requests (PRs), this can be fully automated
    using the
